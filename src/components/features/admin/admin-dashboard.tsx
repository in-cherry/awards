...
import { BarChart3, Users, TrendingUp, Eye, Calendar, DollarSign, Activity, PieChart, Gift, Settings, Instagram, MessageCircle, LifeBuoy } from 'lucide-react';
...
export const AdminDashboard: React.FC<AdminDashboardProps> = ({
  ...
  const urlErrors = {
    ...
    telegramUrl: !isValidUrl(telegramUrl),
    supportUrl: !isValidUrl(supportUrl),
  };
  ...
  const handleSaveSettings = async () => {
    try {
      if (urlErrors.instagramUrl || urlErrors.telegramUrl || urlErrors.supportUrl) {
        alert('Corrija os links inválidos antes de salvar.');
        return;
      }
  ...
  return (
    <div className="min-h-screen flex flex-col text-white">
      ...
      <header className="py-6 px-6 flex items-center justify-between border-b border-white/10 sticky top-0 bg-black/35 backdrop-blur-xl z-50">
        ...
        <p className="text-[10px] text-gray-400 uppercase tracking-widest font-black hidden md:block">
          Navegacao persistente por sidebar
        </p>
      </header>

      <div className="flex-1 max-w-7xl mx-auto px-4 md:px-6 py-6 md:py-8 w-full lg:flex lg:gap-6">
        <AdminSidebar
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div>
                          ...
                          <input
                            ...
                            className={`w-full bg-black/20 border rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none transition-all ${urlErrors.instagramUrl ? 'border-red-500/60 focus:border-red-400' : 'border-white/10 focus:border-emerald-500/50'}`}
                          />
                          {urlErrors.instagramUrl && <p className="mt-1 text-[10px] font-bold text-red-400">URL inválida (use http:// ou https://)</p>}
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">
                            Telegram
                          ...
                          <input
                            ...
                            className={`w-full bg-black/20 border rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none transition-all ${urlErrors.telegramUrl ? 'border-red-500/60 focus:border-red-400' : 'border-white/10 focus:border-emerald-500/50'}`}
                          />
                          {urlErrors.telegramUrl && <p className="mt-1 text-[10px] font-bold text-red-400">URL inválida (use http:// ou https://)</p>}
                        </div>
                        <div>
                          <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mb-2">
                            Suporte
                          ...
                          <input
                            ...
                            className={`w-full bg-black/20 border rounded-xl px-4 py-3 text-white placeholder-gray-500 focus:outline-none transition-all ${urlErrors.supportUrl ? 'border-red-500/60 focus:border-red-400' : 'border-white/10 focus:border-emerald-500/50'}`}
                          />
                          {urlErrors.supportUrl && <p className="mt-1 text-[10px] font-bold text-red-400">URL inválida (use http:// ou https://)</p>}
                        </div>
                      </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2">
                          {instagramUrl && (
                            <a
                              ...
                              className="inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/5 px-3 py-2 text-xs font-semibold text-gray-200"
                            >
                              <Instagram size={14} />
                              Instagram
                            </a>
                          ...
                          {!instagramUrl && !telegramUrl && !supportUrl && (
                            <p className="text-xs text-gray-500">Preencha os links acima para visualizar os botões.</p>
                          )}
                        </div>
                      </div>
                    </div>
